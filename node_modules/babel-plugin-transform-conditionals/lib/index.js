...
function Conditionals(_ref /*:PluginOptions*/) {
  let t = _ref.types;

  return {
    visitor: {
      Conditional: function Conditional(path /* :NodePath */) {
  ...
  if (blockPath.isBlockStatement()) {
    blockPath.traverse({
      VariableDeclaration: function VariableDeclaration(varPath) {
        if (varPath.isVariableDeclaration({ kind: 'var' }) && varPath.scope.getFunctionParent() === blockPath.scope.getFunctionParent()) {
